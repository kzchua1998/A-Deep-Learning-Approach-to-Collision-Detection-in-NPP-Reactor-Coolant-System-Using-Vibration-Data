...
## Datasets
the dataset used consists of simulation time-domain vibration data 
of 2800 collision events generated by 4 accelerometers. Each collision event is 
associated with collision parameters such as collision point, (X, Y) in mm, the mass, 
M (g), and velocity, V (m/s) of the body upon collision to be trained and predicted. 
The locations of the accelerometers in the simulator are enclosed to avoid any intended 
selection bias. Figure 3.2 (Appendix C) shows an example of a simulated collision 
event used in generating the dataset.
